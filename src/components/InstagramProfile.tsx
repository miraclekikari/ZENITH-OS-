...
const InstagramProfile: React.FC<InstagramProfileProps> = ({ profileId, isOwnProfile = false }) => {
  ...
  if (!profile) {
    return <div className="text-center py-8"><p className="text-gray-500">Profile not found or still loading...</p></div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white">
      <div className="relative h-48 md:h-64 bg-gray-200 overflow-hidden">
        ...
        <div className="mt-4">
          <div className="flex items-center gap-2"><h2 className="text-xl font-bold text-gray-900">{profile?.username || 'Username'}</h2>{profile?.is_verified && <div className="w-5 h-5 bg-blue-500 rounded-full flex items-center justify-center"><span className="text-white text-xs">✓</span></div>}</div>
          <div className="flex gap-6 mt-2 text-sm">
              ...
              <span className="font-medium text-gray-900"><strong>{profile?.following_count || 0}</strong> following</span>
          </div>
          <div className="mt-3">
