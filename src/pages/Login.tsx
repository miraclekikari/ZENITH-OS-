...
const SocialButton: React.FC<{ provider: 'google' | 'facebook' | 'github', onError: (msg: string) => void }> = ({ provider, onError }) => {
  ...
  const bgClasses = {
    ...
    facebook: 'bg-[#1877F2]/20 hover:bg-[#1877F2]/30',
...
const Login: React.FC = () => {
  const [authView, setAuthView] = useState('sign-in'); // sign-in, sign-up, magic-link, forgot-password
  ...
  const handleAuthAction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authView === 'sign-up') {
        ...
        if (!error) {
          // In our new flow, Supabase signs the user in automatically. 
          // We can show a temporary message and then navigate, or let onAuthStateChange handle it.
          // To give immediate feedback, we'll navigate to feed with a welcome message.
        ...
        // On successful sign-in, we do NOTHING. The onAuthStateChange listener in App.tsx is the single source of truth and will handle the navigation. This prevents race conditions.
        responseError = error;
        ...
        if (!error) setInfo('Magic Link sent! Check your email to log in.');
        responseError = error;
      } else if (authView === 'forgot-password') {
         const { error } = await supabase.auth.resetPasswordForEmail(formData.email, {
           redirectTo: `${window.location.origin}/reset-password`,
         });
         if (!error) setInfo('Password reset link sent. Check your email.');
         responseError = error;
      }
      ...
      >
        <div className="bg-[#111]/80 backdrop-blur-xl border border-white/[0.08] rounded-2xl p-8 shadow-[0_0_60px_rgba(0,0,0,0.5)]">
          <div className="text-center mb-6">
            ...
            <p className="text-white/25 text-sm mt-2">Secure Gateway v7.1</p>
          </div>

          {error && <motion.div initial={{opacity:0}} animate={{opacity:1}} className="bg-red-500/10 border border-red-500/20 text-red-300 text-xs p-3 rounded-lg text-center font-medium mb-4">{error}</motion.div>}
          ...
          {authView !== 'sign-in' && authView !== 'sign-up' ? null : (
          ...
          {authView === 'sign-in' || authView === 'sign-up' ? <div className="flex items-center gap-3 my-4"><div className="flex-1 h-px bg-white/[0.06]" /><span className="text-white/20 text-xs font-medium">OR</span><div className="flex-1 h-px bg-white/[0.06]" /></div> : null}
          ...
          <div className="mt-6 text-center text-sm">
            ...
            {authView === 'sign-up' || authView === 'forgot-password' || authView === 'magic-link' ? <span className="text-white/25">Remembered your password? <button onClick={() => switchView('sign-in')} className="ml-1 text-emerald-400 font-semibold hover:text-emerald-300 transition-colors">Sign In</button></span> : null}
            {authView === 'sign-in' && <span className="text-white/25">Don't have an ID? <button onClick={() => switchView('sign-up')} className="ml-1 text-emerald-400 font-semibold hover:text-emerald-300 transition-colors">Sign Up</button></span>}
          </div>

           <div className="mt-4 text-center text-sm">
            {(authView === 'sign-in' || authView === 'sign-up') && <button onClick={() => switchView('magic-link')} className="text-yellow-400 hover:text-yellow-300 font-medium transition-colors text-xs flex items-center justify-center gap-1 w-full"><Zap size={12}/> Use a Magic Link</button>}
          </div>
