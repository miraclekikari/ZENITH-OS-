...
import { AtSign, User, Lock, Eye, EyeOff, Loader2, Fingerprint, Key, Zap } from 'lucide-react';
...
const SocialButton: React.FC<{ provider: 'google' | 'github', onError: (msg: string) => void }> = ({ provider, onError }) => {
  ...
  const bgClasses = { google: 'bg-white/[0.08] hover:bg-white/[0.12]', github: 'bg-neutral-800 hover:bg-neutral-700' };
  ...
  return (
    <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={handleLogin} disabled={isLoading} className={`flex-1 flex items-center justify-center gap-2.5 py-3 px-4 rounded-xl font-semibold text-sm transition-all ${bgClasses[provider]} disabled:opacity-60 disabled:cursor-not-allowed text-white`}>
          ...
          <form onSubmit={handleAuthAction} className="space-y-4">
            <div className="relative group">
              ...
              <input 
                ...
                placeholder={isEmailView ? "Email address" : "Email, Username or ID"}
                required 
